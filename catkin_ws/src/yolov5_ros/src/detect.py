...
@torch.no_grad()
def run(weights=ROOT / 'best2.pt',  # model.pt path(s) ###yolov5s.pt
        source=ROOT / '1',  # file/dir/URL/glob, 0 for webcam ##source=ROOT / 'data/images'
    ...
    pub = rospy.Publisher('yolov5_detections', BoundingBoxes, queue_size=10)
    bridge = CvBridge()
    ###
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                for *xyxy, conf, cls in reversed(det):
                    ...
                    box.xmin = int(xyxy[0].item())
                    box.ymin = int(xyxy[1].item())
                    box.xmax = int(xyxy[2].item())
                    box.ymax = int(xyxy[3].item())
                    ros_msg.bounding_boxes.append(box)
            pub.publish(ros_msg)



            # Print time (inference-only)
            # LOGGER.info(f'{s}Done. ({t3 - t2:.3f}s)') ## time check 
